...
DATA_DIR = Path("data")
DATASET_NAME = os.environ.get("DATASET_NAME", "hard_danger_42k_v1")
...
class Layer:
    ...
    late_time_ratio: float | None = None
...
LAYERS = [
    Layer(
        ...
        1,
        int(os.environ.get("DANGER_0_5_GROUPS", "2200")),
        (0.0, 5.0),
        0,
        Q_POINTS,
        float(os.environ.get("DANGER_0_5_LATE_RATIO", "0.55")),
    ),
    Layer(
        "danger_5_10",
        "constructed_danger_5_10km",
        2,
        int(os.environ.get("DANGER_5_10_GROUPS", "3000")),
        (5.0, 10.0),
        100_000,
        Q_POINTS,
        float(os.environ.get("DANGER_5_10_LATE_RATIO", "0.70")),
    ),
    Layer("near_safe", "constructed_near_safe_10_30km", 3, int(os.environ.get("NEAR_SAFE_GROUPS", "1600")), (10.0, 30.0), 200_000, Q_POINTS),
    Layer("mid_safe", "constructed_mid_safe_30_100km", 4, int(os.environ.get("MID_SAFE_GROUPS", "1200")), (30.0, 100.0), 400_000, Q_POINTS),
...
def sample_encounter_time(layer):
    if layer.late_time_ratio is None:
        return np.random.uniform(3600.0, TIME_SPAN)
    if np.random.rand() < layer.late_time_ratio:
        return np.random.uniform(24.0 * 3600.0, TIME_SPAN)
    return np.random.uniform(3600.0, 24.0 * 3600.0)
...
def generate_constructed_group(seed, layer, max_retry_per_point=12):
    ...
    t_enc = sample_encounter_time(layer)
...
def make_metadata(X, Y, G):
    ...
    layer_policy = {
        layer.name: {
            "groups": layer.groups,
            "samples_per_group": layer.samples_per_group,
            "distance_range_km": layer.distance_range_km,
            "late_time_ratio_24h_48h": layer.late_time_ratio,
        }
        for layer in LAYERS
    }
    return {
        ...
        "hard_sample_policy": (
            "Danger samples are rebalanced toward the harder 5-10 km layer, "
            "and dangerous constructed encounters are oversampled in 24-48 h "
            "because previous diagnostics showed larger TCA errors there."
        ),
...
def main():
    ...
    print("Hard-sample policy: more 5-10 km danger samples and more 24-48 h danger encounters.")
    ...
    for layer in LAYERS:
        print(
            ...
            f"late_time_ratio={layer.late_time_ratio}"
        )
